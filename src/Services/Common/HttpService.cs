...
namespace MusicTransify.src.Services.Common
{
    public abstract class HttpService
    {
            ...
            if (_httpClient is not null)
            {
                using (var response = await _httpClient.SendAsync(request))
                {
                    if (response.StatusCode.Equals(HttpStatusCode.OK))
                    {
                        if (response is null)
                        {
                            _logger?.LogError("No response received from {service} for Send request {request}", serviceName, request);
        ...
        public async Task<HttpResponseMessage> GetHttpResponseAsync(string accessToken, string apiBaseUri, string endPoint)
        {
            if (_httpClient is not null)
            {
